...
//
func (r *Repository) UpdatePrincipalInTargets(
	ctx context.Context,
	signer sslibdsse.SignerVerifier,
	targetsRoleName string,
	updatedPrincipals []tuf.Principal,
	signCommit bool,
	...
	updatedIDs := ""
	for _, updated := range updatedPrincipals {
		id := strings.TrimSpace(updated.ID())
		slog.Debug(fmt.Sprintf("Updating principal '%s' in rule file...", id))

		// Check if it exists before removing
		if !targetsMetadata.HasPrincipal(id) {
			return fmt.Errorf("cannot update principal '%s': not found in policy", id)
		}

		// Remove the existing principal and add the new version
		if err := targetsMetadata.RemovePrincipal(id); err != nil {
			return err
		}
		if err := targetsMetadata.AddPrincipal(updated); err != nil {
			return err
		}

		updatedIDs += fmt.Sprintf("\n%s", id)
	}
	...
	slog.Debug(fmt.Sprintf("Signing updated rule file using key '%s'...", keyID))
	env, err = dsse.SignEnvelope(ctx, env, signer)
	if err != nil {
		return err
	}
	...
	commitMessage := fmt.Sprintf("Update principals in policy '%s'%s", targetsRoleName, updatedIDs)

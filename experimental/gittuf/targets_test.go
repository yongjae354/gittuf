...
func TestAddTeamToTargets(t *testing.T) {
	...
	team, err := tufv02.NewTeam("team1", []string{alice.ID(), bob.ID()}, 1)
	assert.Nil(t, err)
	...
	assert.ElementsMatch(t, []string{"alice", "bob"}, teams["team1"].GetMemberIDs())
}
...
func TestUpdateTeamInTargets(t *testing.T) {
	...
	team, err = tufv02.NewTeam("team1", []string{alice.ID(), bob.ID()}, 2)
	assert.Nil(t, err)
	...
	team, err = tufv02.NewTeam("team2", []string{alice.ID(), bob.ID()}, 2)
	assert.Nil(t, err)
	err = r.UpdatePrincipalInTargets(testCtx, targetsSigner, policy.TargetsRoleName, team, false)
	assert.NotNil(t, err)

...
// addPrincipal adds a delegations principal. v02 supports Key and Person
// as principal types. (In this prototype, v02 also supports Team.)
func (d *Delegations) addPrincipal(principal tuf.Principal) error {
	...
	case *Team:
		if err := d.validateTeam(principal); err != nil {
			return err
		}
		d.Principals[principal.ID()] = principal
	default:
...
// Key and Person as principal types. (In this prototype, v02 also supports Team.)
...
// validateTeam verifies that a team is well-formed and that every referenced
// member ID resolves to a Person in the current rule file.
// FYI: currently Team only supports Persons as members.
func (d *Delegations) validateTeam(team *Team) error {
	if team == nil {
		return tuf.ErrInvalidPrincipalType
	}
	if team.ID() == "" {
	...
	if team.GetThreshold() <= 0 {
		return tuf.ErrInvalidThreshold
	}

	memberIDs := set.NewSet[string]()
	for _, memberID := range team.GetMemberIDs() {
		if memberID == "" {
			return tuf.ErrInvalidPrincipalID
		}
		principal, has := d.Principals[memberID]
		if !has {
			return tuf.ErrPrincipalNotFound
		}
		if _, isPerson := principal.(*Person); !isPerson {
			return tuf.ErrInvalidPrincipalType
		}
		memberIDs.Add(memberID)
	}

	if memberIDs.Len() < team.GetThreshold() {
		return tuf.ErrCannotMeetThreshold
	}
...
// removePrincipal removes a delegations principal. v02 supports Key and
// Person as principal types. (In this prototype, v02 also supports Team.)
